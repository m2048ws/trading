# RFC-0006-establish-exact-trade-campaigns: Establish exact single-instrument trade campaigns
...
information, and preserve all existing round-trip and fee-inclusive scenario results. Campaign construction,
...
- Do not calculate venue-reported or estimated trading fees, rebates, commissions, funding, interest, tax, liquidation
  ...
  fee-inclusive round-trip behavior remains unchanged.
...
outcome; campaign membership itself remains non-empty. Independent identity, reference, and per-change validation
failures accumulate deterministically before dependent valuation. The implementation shall preserve input attribution
and exact rational arithmetic and shall not depend on order, scenario, fee, execution, campaign, risk, application,
codec, or runtime types.

`trading-execution-scenario` shall adapt each checked round-trip matched slice to that shared calculation. Its entry and
exit vocabulary, checked flatness, public result, error location, slice ordering, and downstream fee attribution remain
scenario-owned. `ScenarioValuation` and fee-inclusive composition shall not retain a second price-PnL fold. The existing
two-leg facade remains domain-readable even though the underlying economic calculation accepts any finite number of
priced changes.
...
- AC-002 [evidence: automated]: Flat input with non-flat derived exposure, marked input with flat exposure, foreign
  instrument identities or references, and invalid per-change valuation inputs return precise typed failures;
  independently detectable failures accumulate in stable input order and dependent valuation does not run without its
  prerequisite evidence.
- AC-003 [evidence: automated]: Existing single-slice and multi-slice long and short `RoundTripScenario` valuation and
  fee-inclusive evaluation use the shared calculation and remain exactly equal in result, error location, contribution
  order, fee attribution, settlement reference, and rational value to characterized pre-generalization behavior.
- AC-004 [evidence: automated]: Property and completed-artifact tests cover three-or-more-change scale-in and scale-out
  examples, exact zero, input-order-preserving attribution, permutation-invariant totals, and linear traversal cost;
  instrument-economics production and compiler boundaries contain no order, scenario, fee, execution, campaign, risk,
  application, codec, or runtime dependency.
...
- AC-015 [evidence: automated]: A flat authoritative observation uses the flat endpoint, including the exact zero result
  ...
  mark-to-market campaign-relative price PnL. Missing, duplicate, foreign, fill-price-mismatched, incoherent-conversion, or
  wrong-endpoint evidence returns deterministic typed failures and no partial authoritative result.
...
- AC-017 [evidence: automated]: Campaign PnL contains price PnL only and neither invokes hypothetical `FeePolicy` nor
  manufactures fees, funding, account cashflows, or risk; existing scenario fee-inclusive results remain unchanged and
  campaign production classpaths contain no fee-policy or risk dependency.
...
- Migrating round-trip valuation could alter price signs, error ordering, attribution, or downstream fee-inclusive PnL.
  Mitigation: characterize the current public behavior first and require exact example, property, and completed-artifact
  equivalence through the existing fee composition.
