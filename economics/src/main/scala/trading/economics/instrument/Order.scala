...
final case class CheckedActivation[P] private[instrument] (observations: Vector[(String, P)])

sealed trait OrderActivation[P]:
  type Evidence

  def validate(evidence: Evidence): Either[ActivationViolation, CheckedActivation[P]]
  private[instrument] def observations(evidence: Evidence): Vector[(String, P)]

sealed trait TriggerActivation[P] extends OrderActivation[P]

final class ImmediateActivation[P] private[instrument] extends OrderActivation[P]:
  ...
  private[instrument] def observations(evidence: Evidence): Vector[(String, P)] = Vector.empty
...
final case class FixedActivation[P](
  ...
  private[instrument] def checkedEvidence(
    observedPrice: P
  )(
...
object OrderActivation:
  private[instrument] def comparisonSatisfied(
    comparison: TriggerComparison,
    observed: BigInt,
    threshold: BigInt
  ): Boolean =
...
final class FixedTriggerEvidence[P] private[instrument] (
  val reference: PriceReference,
  private[instrument] val comparison: TriggerComparison,
  private[instrument] val triggerPrice: P,
  val observedPrice: P)

final class TrailingTriggerEvidence[P] private[instrument] (
  val reference: PriceReference,
  private[instrument] val comparison: TriggerComparison,
  private[instrument] val offsetTicks: PositiveWhole,
...
final class PegResolution[P] private[instrument] (
  val reference: PriceReference,
  private[instrument] val offsetTicks: BigInt,
...
sealed trait OrderPricing[P]:
  ...
  private[instrument] def observations(resolution: Resolution): Vector[(String, P)]
...
final case class LimitPricing[P](limit: P) extends OrderPricing[P]:
  ...
  private[instrument] def observations(resolution: Resolution): Vector[(String, P)] = Vector.empty
...
final case class PeggedPricing[P](reference: PriceReference, offsetTicks: BigInt) extends OrderPricing[P]:
  ...
  private[instrument] def checkedResolution(
    referencePrice: P,
    resolvedLimit: P
  )(
    ticks: P => BigInt
...
sealed trait OrderExecution[L, P]:
  ...
  def resolve(resolution: Resolution): Either[PricingViolation, EffectivePricing[P]]
  private[instrument] def requiresMaker: Boolean
  private[instrument] def observations(resolution: Resolution): Vector[(String, P)]
...
final case class MarketExecution[L, P](timeInForce: NonRestingTimeInForce) extends OrderExecution[L, P]:
  ...
  private[instrument] val requiresMaker: Boolean = false
...
final case class PricedExecution[L, P, PR <: OrderPricing[P]](
  ...
  private[instrument] def requiresMaker: Boolean = liquidityConstraint == LiquidityConstraint.MakerOnly
...
sealed abstract class Order[L, P] private[instrument]:
...
final class Orders[D <: Dim, B <: Dim, Q <: Dim] private[instrument] (
  ...
  def create[A <: OrderActivation[Price], E <: OrderExecution[Lots, Price]](
    ...
    activation: A,
    execution: E
  ): Either[EconomicsError, Order.Aux[Lots, Price, A, E]] =
    for
  ...
  def stopMarket[A <: TriggerActivation[Price]](
    ...
    trigger: A,
    positionEffect: PositionEffect = PositionEffect.Unrestricted
  ): Either[EconomicsError, Order.Aux[Lots, Price, A, MarketExecution[Lots, Price]]] =
    create(
      intent(side, lots, positionEffect),
      trigger,
