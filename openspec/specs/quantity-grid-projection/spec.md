...
Defines coordinate-backed uniform grids, canonical embedding into exact quantities, and explicit projection back to a
grid. Core `GridQuantity`, `GridRef`, and `UniformGrid` vocabulary belongs to `trading.quantity`; grid relationships,
projection, quantization, constrained encoding, and quotient/remainder and allocation extensions belong to
