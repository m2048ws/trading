...
- [ ] 1.1 Confirm Proposals 0–9 are complete and approved for ordered application and Proposals 0–6 have landed before
  editing risk production code.
...
- [ ] 5.8 Add law/property suites proving totality, exactness, identity preservation, `a <= b => risk(a) <= risk(b)`,
...
- [ ] 6.8 Extend Proposal 2's non-published JMH project with a benchmark-only dependency on `trading-risk`; measure
...
## 8. Scenario/Fee Integration and Migration

- [ ] 8.1 Keep scenario and fee-policy construction out of the risk artifact; add test-only/downstream integration
  examples for compiling fixed exact economic parameters, validating a complete evaluated table, and deliberately
  choosing exhaustive evaluation.
- [ ] 8.2 Migrate existing arbitrary scenario-based sizing call sites through the explicit exhaustive fallback first so
  behavior changes are visible and no callback is silently certified as monotone.
...
- [ ] 9.5 Prepare the validated worktree for fresh independent review without self-certifying, committing, archiving, or
  beginning Proposal 8 outside steward authorization.
