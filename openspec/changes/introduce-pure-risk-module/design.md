...
See `proposal.md` for motivation and the `position-risk-sizing` delta for normative behavior. This is Proposal 7 and
assumes the pure instrument economics foundation from Proposal 4. Proposals 5 and 6 provide pure scenario and fee-policy
capabilities, but this revision deliberately does not make those modules dependencies of the sizing kernel.
...
After moving risk production and tests, the transitional `economics` SBT project from Proposal 4 is empty and is
deleted. Cross-module examples belong in test-only integration/adversarial code; no umbrella production artifact is
retained.

Proposal 2's non-published `benchmarks` project gains a benchmark-only dependency on `trading-risk`; `trading-risk`
...
Proposal 6's fee-inclusive evaluation can still feed sizing, but it does not automatically prove a curve law. Three
...
1. Observe the portfolio gate and land Proposals 0–6 before applying this change.
...
15. Reconcile OpenSpec artifacts if a semantic issue is discovered; otherwise prepare for fresh independent review
    without self-certification or Proposal 8 work.

Rollback before release is a source/build revert to the Proposal 6 boundary; no persisted schema or external state is
changed.
