...
#### Scenario: Consume pure economics above its foundations
...
#### Scenario: Construct positive lots purely
...
#### Scenario: Represent signed position independently of side
...
#### Scenario: Validate additional conversions together
...
#### Scenario: Require an explicit third-asset conversion
...
#### Scenario: Construct a structurally valid local value directly
...
#### Scenario: Apply a typed conversion after heterogeneous selection
...
**Reason**: Instrument-owned capability views make the foundational value depend conceptually on order, execution,
fee-policy, and risk concerns that should consume it.

**Migration**: Keep intrinsic observers on their values; replace `instrument.prices`, `instrument.market`, and
`instrument.valuation` with pure `Price`, `MarketState`, and valuation entry points that accept the instrument; replace
`instrument.orders`, `instrument.scenarios`, `instrument.fees`, and `instrument.sizing` with the downstream APIs defined
