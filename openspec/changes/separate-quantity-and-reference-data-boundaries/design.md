...
`GridDefinition` binds that full identity to one positive rational quantum. This proposal specifies immutable values;
the next proposal specifies append-only registration and conflict behavior.
...
```scala
sealed trait DimensionHandle[D <: Dim]:
  def key: DimKey
  def ref: DimRef[D]
...
a stable key to an arbitrary grid, or forge a handle by declaring the package. The token does not become another public
type parameter. The next proposal ensures every immutable catalog state and snapshot in one live catalog lineage
