...
#### Scenario: Defer issuer checks to reference data
...
#### Scenario: Keep an exact runtime result in memory
...
#### Scenario: Project before a grid-coordinate boundary
...
#### Scenario: Serialize a runtime dimension carrier
- **WHEN** a caller passes an authority-bearing runtime dimension package to `ObjectOutputStream`
- **THEN** serialization fails without producing a supported persistence payload

#### Scenario: Use explicit downstream decoding
...
#### Scenario: Transform a checked runtime value
...
#### Scenario: Preserve a dimension-changing witness
...
#### Scenario: Reject unchecked coordinate attachment
...
#### Scenario: Keep runtime authority non-extractable
...
#### Scenario: Construct a rate from runtime dimensions
...
#### Scenario: Reconcile independently obtained equal endpoints
