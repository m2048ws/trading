...
Public mathematical and domain APIs SHALL represent expected absence, invalidity, conflict, and failure in their result types. They MUST NOT use `null`, unchecked extraction, sentinel values, or exceptions as ordinary control flow. Unavoidable partial operations, casts, and mutable mechanisms SHALL be narrowly scoped, explicitly named or hidden, protected by a stated invariant, and inaccessible as public construction authority.
...
- **WHEN** Scala cannot express a relationship already established by authoritative runtime equality or sealed evidence
- **THEN** the cast is isolated behind that checked boundary and adversarial tests verify callers cannot use it to forge authority

#### Scenario: Runtime state must mutate

- **WHEN** a live interpreter must coordinate changing state
- **THEN** mutation remains encapsulated by the runtime effect abstraction while the state-transition semantics remain testable as pure immutable logic where practical
