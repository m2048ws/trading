...
`SameDimension` to align operands whose static dimension types differ. Grid-by-grid multiplication and mixed grid/exact
...
nonzero check. Exact operations that leave a grid SHALL use the same static normalization rules as ordinary `Quantity`
arithmetic and SHALL NOT retain a separate grid-specific dimension algebra.
