...
See `proposal.md` for motivation and the `order-scenarios` delta for normative behavior. This is Proposal 5 in the
portfolio. It assumes Proposal 4 has established `trading-instrument-economics`, removed instrument-owned capability
...
- Do not introduce an order owner token, generative order identity, or an `OrderId` merely to associate scenario
  evidence.
- Do not make order/scenario values persistence schemas; Proposal 9 owns stable encodings.
...
`executionScenario`. Proposals 6 and 7 remove those remaining concerns. Completed-JAR compiler tests enforce that
...
their associated evidence constructors before it can build assumptions. Proposal 9 specifies the stable codec; this
...
to Proposal 6's fee policy or a future venue adapter. This avoids encoding a venue taxonomy in the order algebra.
...
1. Observe the all-proposals-before-apply gate and land Proposals 0–4 first.
...
11. Reconcile OpenSpec artifacts if Scala constraints require a semantic change; otherwise prepare for fresh independent
    review without self-certification or implementation of Proposal 6.

Rollback before release is a source revert to the Proposal 4 boundary; this proposal changes no external state or
persistence format.
