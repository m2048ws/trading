...
final case class AppliedCommandTransition[D <: Dim, B <: Dim, Q <: Dim] private[execution] (
  state: CommandState[D, B, Q])
  extends CommandTransition[D, B, Q]

final case class IdempotentCommandTransition[D <: Dim, B <: Dim, Q <: Dim] private[execution] (
  state: CommandState[D, B, Q])
  extends CommandTransition[D, B, Q]

final case class ConflictingCommandTransition[D <: Dim, B <: Dim, Q <: Dim] private[execution] (
  state: CommandState[D, B, Q])
  extends CommandTransition[D, B, Q]

final case class ConflictingDispatchTransition[D <: Dim, B <: Dim, Q <: Dim] private[execution] (
  state: CommandState[D, B, Q])
  extends CommandTransition[D, B, Q]

final case class RejectedCommandTransition[D <: Dim, B <: Dim, Q <: Dim] private[execution] (
  state: CommandState[D, B, Q],
      ...
      state.issuedCommands.get(command.commandId) match
        ...
        case None =>
          ...
          CommandViolations.from(scope ++ reference) match
            case Some(violations) => RejectedCommandTransition(state, violations)
            case None             =>
    ...
    if evidence == null then
      ...
      CommandViolations.from(scope ++ reference) match
        ...
        case None             =>
          ...
          if current.contains(evidence) then IdempotentCommandTransition(state)
          else
            ...
            if conflicting then ConflictingDispatchTransition(next) else AppliedCommandTransition(next)
end CommandState
