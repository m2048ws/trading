...
interpreters; this first module-boundary checkpoint adds no speculative runtime API.
...
owners. The target physical instrument-economics, order-model, execution-scenario, fee-policy, risk, and
boundary-codec responsibilities remain active proposals. Proposal 8 owns the first live catalog interpreter; Proposal
9 owns the removed packing capability's durable replacement. The runtime module exists because it owns a concrete
effect dependency and independently verified publication boundary, not merely to match the target diagram.
