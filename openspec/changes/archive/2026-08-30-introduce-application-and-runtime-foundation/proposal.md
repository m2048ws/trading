...
The merged quantities/reference-data foundation already keeps catalog semantics pure, but the first genuinely live
capability—catalog publication—still needs a concrete home and execution model.
...
- Establish the existing `trading-application` artifact as the home for effect-polymorphic workflows and the smallest
...
- Extend the existing reusable capability contract testing so every future live, simulation, backtest, or deterministic
...
- The existing `application` project (`trading-application`, package `trading.application`) remains dependent only on
...
- The existing non-published JMH project gains benchmark-only runtime/application dependencies for snapshot capture,
...
- The existing `LiveCatalog[F]` API remains the application-facing contract; this change provides its concrete runtime
