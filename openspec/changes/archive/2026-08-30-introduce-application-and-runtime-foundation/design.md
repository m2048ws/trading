...
`RFC-0002-architecture-portfolio/S-01-application-runtime-foundation` and is independent of the other portfolio Slices.
The merged repository baseline already contains the pure quantity/reference-data foundation, `trading-application`,
the minimal `LiveCatalog[F]` port, its pure semantic model, and an interpreter-conformance harness; it intentionally
supplies no live interpreter.
...
`S-05-versioned-boundary-codecs` may proceed separately with boundary representations and batch decode/replay. It may
use the application/runtime placement rules here but must not turn codecs into effects or serialize trusted catalog
handles.
...
The merged baseline already contains `application/`, artifact `trading-application`, package `trading.application`, depending only
...
The exact names follow the existing catalog/application contract. The function supplied to `modify` is total and pure, so CAS reevaluation is safe.
...
### 12. Reuse the existing conformance harness and extend it at the runtime boundary

The existing application test sources contain a parameterized `LiveCatalog` semantic contract. Runtime tests depend on
...
outcomes rather than scheduler-specific interleavings. The existing non-published JMH project is extended with
...
- [Fresh in-memory roots cannot provide restart continuity] → Persist stable commands/IDs, create a new lineage on
  restart, and resolve through the new snapshot as required by the reference-data and boundary-codec contracts.

## Migration Plan

1. Verify the accepted RFC/source/traceability bindings and the merged runtime-neutral application artifact, pure
   catalog model, `LiveCatalog[F]`, and shared conformance harness. No other RFC-0002 Slice is an implementation
   prerequisite.
...
6. Reconcile existing forward references so this Slice is described as the admission/runtime foundation rather than
...
8. Run formatting, clean dependency-ordered compilation, focused/runtime/conformance tests, explicit JMH compilation/
   measurement, completed-JAR compiler checks, adversarial tests, the full repository validation matrix, and prepare
   evidence for the separate canonical Corgi Verify and human-review gates.
9. Keep boundary decode/replay outside this delivery; its independent Slice can adopt the accepted snapshot boundary
   when integration ordering requires it.

Rollback is a source-level revert of the new runtime project and forward-reference documentation. No durable schema,
external resource, or deployment is created; the pure catalog and application port remain usable with
