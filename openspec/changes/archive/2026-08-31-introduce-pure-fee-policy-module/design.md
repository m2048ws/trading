...
See `proposal.md` for motivation and the fee/scenario deltas for normative behavior. This delivery is
`RFC-0002-architecture-portfolio/S-04-fee-policy`. `S-02-order-execution-scenarios` and `S-03-pure-risk` are now closed.
S-03 provisionally established the physical fee-policy boundary so it could retire the transitional economics artifact;
this Slice owns its semantic redesign.

The delivered `trading-fee-policy` baseline owns `FeeRate`, raw-sign-validated percentage/minimum helpers,
`FeeSchedule`, and `FeeLine`. A line still contains both a caller-supplied slice index and caller-supplied market state.
Later valuation receives the scenario again, checks index bounds, then uses JVM `eq` to verify that the supplied market
is the one at that index. `FeePolicy.pnl` also owns scenario traversal, schedule evaluation, attribution validation, fee
conversion, price PnL, and core aggregation.

Instrument economics owns denomination/fee values and core `PricePnl`/`SettledFeeContribution`/`Pnl`; execution
scenarios own complete scenarios and non-empty matched slices. The remaining task is to give policy and scenario
attribution an honest downstream model without putting either back into the core.
...
### 1. Finalize the existing fee-policy artifact
...
The finalized artifact owns policy math, the open policy strategy, policy composition, directives, validated scenario fee
...
The delivered baseline already has separate `risk` and `feePolicy` artifacts and no transitional `economics` artifact.
This Slice replaces the provisional fee-policy API while preserving risk as an independent lower layer. Completed-JAR
tests prove the core and scenario artifacts do not acquire a reverse dependency on fee policy.
...
upward scenario dependency removed by the instrument-economics boundary.
...
1. Confirm the effective RFC/source/traceability bindings and reconcile this package against the closed S-02 and S-03
   deliveries before claim.
2. Audit and finalize the existing `feePolicy` boundary, direct dependencies, completed-JAR task, root aggregation, and
   forbidden-dependency compiler checks.
3. Refine the existing `FeeRate` mathematics and replace raw-sign validation with refined typed inputs and exact
   quantity operations.
...
8. Migrate fee-policy integration fixtures and downstream examples without adding a production dependency from risk to
   fee policy or changing the separately owned pure-risk Slice's search behavior.
...
11. Retain the reconciled planning revision through implementation; if a later baseline change invalidates an
    assumption, reconcile before affected work. Prepare evidence for separate canonical Verify and human review
    without starting the boundary-codec Slice.

Rollback before release is a source revert to the reconciled, closed pure-risk delivery baseline; no persisted schema,
external policy store, or runtime state is changed.
