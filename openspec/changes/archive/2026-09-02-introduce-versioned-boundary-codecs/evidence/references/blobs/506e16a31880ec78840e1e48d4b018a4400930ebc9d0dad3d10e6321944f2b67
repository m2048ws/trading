...
See `proposal.md` for motivation and the two delta specifications for normative behavior. This delivery is
`RFC-0002-architecture-portfolio/S-05-versioned-boundary-codecs` and is blocked until
`S-02-order-execution-scenarios` is closed. That delivered baseline supplies immutable append-only catalog
state/snapshots, a proof-carrying instrument assembly boundary, instrument economics, and algebraic order/scenario
models. The application/runtime, fee-policy, and pure-risk Slices are not prerequisites and are not dependencies.

The merged quantity/reference-data boundary removes `PackedGridQuantity`, `PackedAssetGridQuantity`, and
registry-backed decoding rather than blessing an unversioned logical record. The merged reference-catalog boundary
states that stable catalog commands may be replayed but handles/snapshots are never authority-bearing persistence. The
merged instrument-assembly boundary makes `InstrumentDefinition` stable-ID domain data but explicitly not a wire
schema. The delivered economic and order/scenario boundaries likewise keep domain values independent of Java
...
repository's JDK 25 minimum build/runtime baseline supports Jackson 3.x. Jackson is a syntax mechanism behind a package-private
...
`InstrumentDefinitionRecord.V1` structurally mirrors the instrument-assembly boundary's stable-ID definition product. The record codec first
...
Alternative considered: permit unchanged entries as heartbeat revisions. Rejected because the reference-catalog contract defines revisions
...
- [Scenario market records can drift from MarketState observers] → Require the instrument-economics boundary's public base/quote/additional
...
1. Confirm the effective RFC/source/traceability bindings and require the order/scenario Slice to be closed; reconcile
   final module names, reference-data identities/snapshots, instrument definition/assembly, economic observers, and
   order/scenario alternatives against that delivered boundary before claim.
...
12. Run formatting, clean dependency-ordered compilation, all focused/property/compiler/adversarial/external-artifact
    tests, focused JMH smoke runs, the full repository matrix, strict Corgi contract integrity, and portfolio-wide
    consistency review before the final Task Group checkpoint.
13. Prepare the final acknowledged Task Group commit and evidence for separate canonical Verify, human review, human
    QA, and Archive; do not add storage/runtime integrations or begin another Slice.
