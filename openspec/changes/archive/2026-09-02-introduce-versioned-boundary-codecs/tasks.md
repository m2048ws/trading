...
- [ ] 1.1 Confirm the effective RFC/source/traceability bindings, the order/scenario Slice is closed, this package has
  been reconciled against its delivered boundary, and the JDK 25 minimum is effective before codec production edits.
- [ ] 1.2 Refresh Git/source/build/Corgi state and record a clean baseline for formatting, compilation, catalog/
...
## 12. Verification Evidence and Corgi Handoff
...
- [ ] 12.3 Inspect packaged APIs and production/test dependency reports for leaked parser/Cats validation internals,
  ...
  authority.
- [ ] 12.4 Confirm the current Slice still satisfies strict planning/source/traceability integrity, perform the final
  portfolio-wide requirement/dependency/name/order cross-check, and reconcile implementation drift before the final
  Task Group checkpoint.
- [ ] 12.5 Prepare the final acknowledged Task Group commit and evidence for separate canonical Verify, human review,
  human QA, and Archive without claiming storage durability or beginning another Slice.
