...
final case class DimensionEvidenceMismatch(left: DimensionKey, right: DimensionKey) extends RuntimeEvidenceError
...
final case class ForeignRegistryEvidence(left: DimensionKey, right: DimensionKey) extends RuntimeEvidenceError
...
object RuntimeEvidence:
  ...
  def sameDimension(l: DimensionWitness, r: DimensionWitness): Either[RuntimeEvidenceError, SameDimension[l.D, r.D]] =
    if !l.dimension.sharesRegistryWith(r.dimension) then
      ...
      SameDimension
        .between(l.dimension.asDimensionRef, r.dimension.asDimensionRef)
        .toRight(DimensionEvidenceMismatch(l.dimension.key, r.dimension.key))

  def sameGrid[A <: Dimension, B <: Dimension](
