...
object ClosedDimensionCaller:
  ...
  val rawNotional: Quantity[Times[BTC, Divide[USD, BTC]]] = amount * btcToUsd
  val notional: Quantity[USD] = amount.applyRate(btcToUsd)
  ...
  val identity: DimRef[Divide[USD, USD]] = DimRef.divide(usd, usd)
